...
class Capture(threading.Thread):
    ...
    def _selected_backend(self):
        ...
        if len(enabled) != 1:
            logger.error("[截取] 请且仅启用一种截取后端: BetterCam、OBS 或 MSS。")
            return None
        return enabled[0]
    ...
    def capture_frame(self):
        ...
        if bc is not None:
            try:
                ...
                if self.running:
                    logger.warning(f"[截取] BetterCam 帧读取失败: {e}")
    ...
    def _center_region(self, screen_width, screen_height, width, height, x_offset, y_offset):
        if width > screen_width or height > screen_height:
            raise ValueError(
                f"[截取] 检测窗口 {width}x{height} 大于显示器分辨率 {screen_width}x{screen_height}。"
            )
    ...
    def get_primary_display(self):
        ...
        if not monitors:
            raise RuntimeError("[截取] 未找到任何显示器。")
    ...
    def find_obs_virtual_camera(self):
        ...
        for i in range(max_tested):
            ...
            if cap.getBackendName() == obs_camera_name:
                logger.info(f'[截取] 在索引 {i} 处找到 OBS 虚拟摄像头')
                cap.release()
                return i
            cap.release()
        return -1
    ...
    def print_startup_messages(self):
        ...
        try:
            ...
            logger.info(f'读取版本文件出错: {str(e)}')

        logger.info(f"""
Watermelon Aimbot 已启动! (版本 {version})
热键:
[{cfg.hotkey_targeting}] - 瞄准目标
[{cfg.hotkey_exit}] - 退出程序
[{cfg.hotkey_pause}] - 暂停瞄准
[{cfg.hotkey_reload_config}] - 重载配置
""")
