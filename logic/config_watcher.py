...
class Config:
    ...
    def __init__(self, path: StrPath = "config.ini", window_names_path: StrPath = "window_names.txt"):
        ...
        self.DEFAULTS: Dict[str, Dict[str, str]] = {
            ...
            "Aim": {
                ...
                "prediction_jitter_threshold": "5.0",
                "prediction_max_offset": "15.0",
                "aim_color_centroid": "true",
            },
            "Hotkeys": {
                "hotkey_targeting": "LeftControl",
                "hotkey_exit": "F2",
                "hotkey_pause": "F3",
            ...
            "Recoil": {
                "recoil_enable": "false",
                "recoil_scale": "1.0",
                "recoil_fire_rate": "9.75",
                "recoil_duration": "2.7",
                "recoil_pattern": "0,12; 0,18; 0,24; 0,21; 0,19; 0,18; 0,16; 0,15; 0,13; 0,12; 0,10; 0,9; 0,7; 0,7; 0,6; 0,6; 0,4; 0,4; 0,4; 0,3; 0,3; 0,3; 0,3; 0,1.5; 0,1.5; 0,1.5; 0,1; 0,1; 0,1; 0,1",
            },
            "Arduino": {
                "arduino_move": "false",
                "arduino_shoot": "false",
            ...
            "CH9329": {
                ...
                "ch9329_screen_width": "auto",
                "ch9329_screen_height": "auto",
            },
            "AI": {
                "ai_model_use_enc": "true",
            ...
            "Debug window": {
                ...
                "log_debug_raw_input": "false",
                "log_debug_aim": "false",
                "log_debug_timing": "false",
            },
        }
    ...
    def read(self, verbose: bool = False) -> None:
        ...
        if not self.CONFIG_PATH.exists():
            logger.warning("[配置] config.ini 未找到，正在使用默认配置创建。")
        ...
        try:
            ...
            logger.error(f"[配置] 读取 config.ini 失败: {e}。正在使用默认配置重新初始化。")
            self._init_with_defaults()
    ...
    def _save(self) -> None:
        try:
            ...
            with self.CONFIG_PATH.open("w", encoding="utf-8") as f:
                self.config.write(f)
            self._last_mtime_ns = self._get_mtime_ns()
            logger.info("[配置] 已保存 config.ini")
        except Exception as e:
            logger.error(f"[配置] 保存 config.ini 失败: {e}")
    ...
    def _ensure_and_get(
        ...
        try:
            ...
            logger.warning(f"[配置] [{section}] {option!r}={raw!r} 值无效，已重置为默认值。")
            self.config.set(section, option, self._to_str(fallback))
            self._dirty = True
    ...
    def _bind_attributes(self) -> None:
        ...
        self.body_x_offset = self._ensure_and_get(s, "body_x_offset", 0.0, float)
        ...
        self.prediction_jitter_threshold = self._ensure_and_get(s, "prediction_jitter_threshold", 3.0, float)
        self.prediction_max_offset = self._ensure_and_get(s, "prediction_max_offset", 15.0, float)
        self.aim_color_centroid = self._ensure_and_get(s, "aim_color_centroid", True, self._to_bool)

        # Hotkeys
        s = "Hotkeys"
        self.hotkey_targeting = self._ensure_and_get(s, "hotkey_targeting", "LeftControl", str)
        ...
        self.mouse_dead_zone = self._ensure_and_get(s, "mouse_dead_zone", 2, int)
        ...
        # Recoil
        s = "Recoil"
        self.recoil_enable = self._ensure_and_get(s, "recoil_enable", False, self._to_bool)
        self.recoil_scale = self._ensure_and_get(s, "recoil_scale", 1.0, float)
        self.recoil_fire_rate = self._ensure_and_get(s, "recoil_fire_rate", 9.75, float)
        self.recoil_duration = self._ensure_and_get(s, "recoil_duration", 2.7, float)
        recoil_pattern_str = self._ensure_and_get(s, "recoil_pattern", "", str)
        self.recoil_pattern = self._parse_recoil_pattern(recoil_pattern_str)
        ...
        # CH9329
        s = "CH9329"
        self.ch9329_move = self._ensure_and_get(s, "ch9329_move", False, self._to_bool)
        self.ch9329_shoot = self._ensure_and_get(s, "ch9329_shoot", False, self._to_bool)
        self.ch9329_port = self._ensure_and_get(s, "ch9329_port", "auto", str)
        self.ch9329_baudrate = self._ensure_and_get(s, "ch9329_baudrate", 9600, int)
        self.ch9329_screen_width = self._ensure_and_get(s, "ch9329_screen_width", "auto", str)
        self.ch9329_screen_height = self._ensure_and_get(s, "ch9329_screen_height", "auto", str)

        # AI
        s = "AI"
        self.ai_model_use_enc = self._ensure_and_get(s, "ai_model_use_enc", True, self._to_bool)
        ...
        self.log_debug_raw_input = self._ensure_and_get(s, "log_debug_raw_input", False, self._to_bool)
        self.log_debug_aim = self._ensure_and_get(s, "log_debug_aim", False, self._to_bool)
        self.log_debug_timing = self._ensure_and_get(s, "log_debug_timing", False, self._to_bool)
    ...
    @staticmethod
    def _parse_recoil_pattern(s: str):
        """
        解析后坐力补偿曲线字符串为 [(dx, dy), ...] 列表。
        格式: "dx,dy; dx,dy; ..."，例如 "0.0,0.8; 0.0,1.2; -0.3,1.3"
        解析失败的条目会被跳过，整体解析失败返回空列表。
        """
        ...
        for pair in s.split(";"):
            ...
            parts = pair.split(",")
            if len(parts) != 2:
                logger.warning(f"[配置] 后坐力曲线格式错误，跳过: '{pair}'（应为 dx,dy）")
                continue
            try:
                ...
                logger.warning(f"[配置] 后坐力曲线数值无效，跳过: '{pair}'")
        return pattern
    ...
    def _get_random_window_name(self) -> str:
        try:
            ...
            logger.warning("[配置] window_names.txt 未找到，使用默认窗口名称。")
            return "Calculator"
        except Exception as e:
            logger.error(f"[配置] 读取 window_names.txt 失败: {e}。使用默认名称。")
            return "Calculator"
