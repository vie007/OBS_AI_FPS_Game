...
class ArduinoMouse:
    def __init__(self):
        ...
        try:
            ...
            logger.info(f'[Arduino] 已连接! 端口: {self.serial_port.port}')
        except Exception as e:
            logger.error(f'[Arduino] 未连接...\n{e}')
            self.checks()
    ...
    def checks(self):
        for process in psutil.process_iter(['pid', 'name']):
            if process.info['name'] == 'Arduino IDE.exe':
                logger.error('[Arduino] Arduino IDE 正在运行，请关闭 IDE 后重启程序。')
                break
        ...
        try:
            ...
            if USB_Host_Shield_library_path is None:
                logger.error("[Arduino] 未找到 USB Host Shield 库")
                return
                    ...
                    for line in file:
                        if line.startswith('#define ENABLE_UHS_DEBUGGING'):
                            ...
                            if len(parts) == 3 and parts[1] == 'ENABLE_UHS_DEBUGGING':
                                value = parts[2]
                                if value == '1':
                                    logger.error(f'[Arduino] 请在 {hid_settings} 文件中禁用 `ENABLE_UHS_DEBUGGING` 设置。')
                                    break
        except Exception as e:
            logger.error(f'[Arduino] 未找到 USB_Host_Shield 库。\n{e}')
